...
class BST {
	...
	public:
		...
		void deleteNode(T d);
		void serializePreOrder(ofstream& writer);
	...
	private:
		...
		void serializePreOrderPrivate(TreeNode<T>* node, ofstream& writer);
};
...
TreeNode<T>* BST<T>::createLeaf(T d) {
	//cout << "in createLeaf" << endl;
	TreeNode<T>* newNode = new TreeNode<T>(d);
	size++;
	//cout << "leaf created" << endl;
	//cout << d.advisees << endl;
	//cout << newNode->data.advisees << endl;
...
TreeNode<T>* BST<T>::getParent(T d) {
	return getParentPrivate(d,root);
}
